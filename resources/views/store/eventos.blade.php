...
@include('store.informativa.layouts.headContentEventos')
								...
								<div class="panel panel-default">
									<div class="panel-body">
										...
										@if(isset($evento->url_img))
											<img class="img-responsive" src="{{ asset('images/eventos/'.$evento->url_img) }}">
										@endif
										<br>
										<i>Fecha: {{ explode(" ", $evento->event_date)[0] }}<br>
										Sede:
											 <td>{{ $evento->lugar }}</td></i>
										<br>
										...
										@if(isset($evento->url_document))
											<br>
											<p>
												<a class="download_file" href="{{ asset('/file/getEvento/'.$evento->url_document) }}" target="_blank"><i class="fa fa-download" aria-hidden="true"></i><span>&nbsp;{{ $evento->url_document }}</span></a>
		...
		<script type="text/javascript">
			loadEvents(<?php echo json_encode($eventosAll); ?>);
		</script>		
@stop
